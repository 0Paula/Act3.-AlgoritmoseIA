...
Ninguno de los genes sigue una distribución normal.
...
tabla_cargas <- cargas_df %>%
  rownames_to_column(var = "Gen") %>% # Crea la columna Variable
  select(Gen, PC1, PC2,PC3,PC4,PC5,PC6) %>%
...
#Gráfica de variables según calidad de representación

fviz_pca_var(pca, col.var="cos2",geom=c("arrow","text"))+
  ggtitle("PCA - Variables (cos2)") +
  theme_classic()

# Clustering variables basado en las coordenadas PCA

set.seed(1234)

vars <- get_pca_var(pca) # Extraer información de las variables 
km_pca_var<- kmeans(vars$coord, centers = 3, nstart = 25) # K-means sobre las variables
...
#Gráfica de los individuos en función de las categorías de IMC

imc_cat <- cut(df_pca$imc_kg_m2, # Crear un vector con las categorías de IMC
